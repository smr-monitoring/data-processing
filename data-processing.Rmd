...
Read in data
```{r}
licor <- read_csv("raw_data/licor.csv")
nep <- read_csv("raw_data/nep.csv")
secchi <- read_csv("raw_data/secchi.csv") %>%
  select(sample_date, secchi_depth_m)
tss <- read_csv("raw_data/tss_calc.csv") %>%
  select(sample_date, mg_l_tss)
ysi <- read_csv("raw_data/ysi.csv") %>%
...
```{r}
nppdat <- nep %>%
  mutate(initial_datetime = ymd_hms(paste(initial_date, initial_time, sep = " ")),
         final_datetime = ymd_hms(paste(final_date, final_time, sep = " ")),
  ...
  pivot_wider(id_cols = c("initial_date", "initial_datetime","final_datetime", "replicate", "depth_m", "depth", "depth_m"), names_from = treatment, values_from = do_rate_mg_l_h) %>%
  mutate(npp = light,
         r = dark,
         gpp = light - dark,
...
intfun <- function(dat) {
  ...
  fluxtable <- left_join(fluxtable, sun) %>%
    ...
    npp_g_o2_m2_d = npp_g_o2_m2_h * daylight_h,
    gpp_g_o2_m2_d = gpp_g_o2_m2_h * daylight_h,
    r_g_o2_m2_d = r_g_o2_m2_h * 24,
    npp_g_c_m2_d = npp_g_c_m2_h * daylight_h,
...
```{r}
save(kd, npp_avg, secchi, tss, ysi, file = "data_processed/water_qual_processed.RData")
